...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const web3Modal = new Web3Modal({
        ...
        projectId,
        metadata,
        enableAnalytics: false // Optional - To disable analytics
    });
    ...
    // --- CORE FUNCTIONS ---
    const updateUIForConnection = (account) => { const shortAddress = `${account.substring(0, 6)}...${account.substring(account.length - 4)}`; connectButtons.forEach(btn => { btn.textContent = shortAddress; btn.disabled = true; }); if (walletConnectionDiv) walletConnectionDiv.classList.add('hidden'); if (walletConnectedDiv) walletConnectedDiv.classList.remove('hidden'); if (walletAddressP) walletAddressP.textContent = account; if (mintBtn) mintBtn.disabled = false; if (swapActionButton) swapActionButton.textContent = "Swap"; };
    const updateUIForDisconnection = () => { connectButtons.forEach(btn => { btn.textContent = "Connect Wallet"; btn.disabled = false; }); if (walletConnectionDiv) walletConnectionDiv.classList.remove('hidden'); if (walletConnectedDiv) walletConnectedDiv.classList.add('hidden'); if (mintBtn) mintBtn.disabled = true; if (swapActionButton) swapActionButton.textContent = "Connect Wallet"; connectedAccount = null; provider = null; signer = null; contract = null; };
    const connectWallet = async () => { try { const modalProvider = await web3Modal.open(); provider = new ethers.providers.Web3Provider(modalProvider); signer = provider.getSigner(); connectedAccount = await signer.getAddress(); updateUIForConnection(connectedAccount); modalProvider.on("accountsChanged", (accounts) => { if (accounts.length > 0) { connectedAccount = accounts[0]; updateUIForConnection(connectedAccount); } else { disconnectWallet(); } }); } catch (error) { console.error("Could not connect wallet:", error); updateUIForDisconnection(); } };
    const disconnectWallet = async () => { await web3Modal.disconnect(); updateUIForDisconnection(); };
    ...
    const mintNFT = async () => {
        ...
        try {
            ...
            if (error.code === 'INSUFFICIENT_FUNDS') { alert('Minting failed: You do not have enough tBNB for the mint price + gas.'); } else if (error.data && error.data.message) { alert(`Minting failed: ${error.data.message}`); } else { alert('Minting failed. The transaction may have been rejected or failed. Check the console.'); }
        } finally {
